...
export const getAllPost = async (req, res) => {
  try {
    const post = await Post.find().populate('user', 'name email').sort({ date: -1 });
    res.status(200).json({ message: 'Posts fetched successfully', success: true, post });
  } catch (error) {
    console.log(error);
    return res.json({ message: 'Internal server error', success: false });
  }
...
export const getUserPosts = async (req, res) => {
  try {
    ...
    const post = await Post.find({ user: userId })
      ...
      .sort({ date: -1 });
    res.status(200).json({ message: 'User posts fetched successfully', success: true, post });
  } catch (error) {
    console.log(error);
    return res.json({ message: 'Internal server error', success: false });
  }
...
export const editPost = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Post updated', success: true, post });
  } catch (error) {
    console.log(error);
    return res.json({ message: 'Internal server error', success: false });
  }
